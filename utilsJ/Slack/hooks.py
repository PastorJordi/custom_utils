...
class whisp:
    ...
    def whisper(self, dst, text):
        ...
        try:
            ...
            print(
                f"couldnt whisper that, probably user is not in contact list ({self.contacts.keys()})"
            )
        ...
        if uploadpath is not None:
            ...
            if os.path.exists(uploadpath):
                ...
                response = client.files_upload(
                    channels=channel, file=uploadpath, initial_comment=message
                )
            else:
    ...
    def webmsg(self, channel, message):
        ...
        try:
            ...
            if channel in self.hooks.keys():
                data = {"text": message}
                requests.post(self.hooks[channel], json.dumps(data))
            else:
