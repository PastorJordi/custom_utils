...
from utilsJ.Models import traj, simul
...
def com_matrix_comparison_silent_vs_nonsilent(df):
    ...
    subset=df.dropna(subset=["avtrapz", "allpriors", "CoM_sugg", "special_trial"])
...
def pCoM_vs_coh(df, subject):
    ...
    """
    ev_vals = np.sort(np.abs(df.coh2.unique())) # unique absolute value coherence values
    if subject == 'all':
        ...
        for i, subj in enumerate(df.subjid.unique()):
            ...
            silent = df_sub.loc[df_sub.special_trial == 2] # separating silent from nonsilent
