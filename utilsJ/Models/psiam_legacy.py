...
def D2M_simulation(
    ...
    Rep_vec = Block_vec * (rng.binomial(1, prep_rep, n) * 2 - 1) + (
        Block_vec - 1
    ) ** 2 * (rng.binomial(1, prep_alt, n) * 2 - 1)
    ...
    Dif = rng.choice(
        [0, 1, 2, 3], size=n
    )  # idk which is which # consider switching to 0, .25, .5, 1
    ...
    for i in tqdm.tqdm_notebook(range(1, n)):
        vv_u = (
            v_u
            + prop["v_trial"]
            * prop["trial"][1, (prop["trial"][0, :] < rr[i - 1]).sum()]
        )
        vv_u += (
            prop["v_rewsize"]
            * prop["rewardsize"][1, (prop["rewardsize"][0, :] < ss[i - 1]).sum()]
        )
        vv_u += (
            prop["v_cumrew"]
            * prop["cumreward"][1, (prop["cumreward"][0, :] < uu[i - 1]).sum()]
        )
        ...
        x_u = 0
        x_e = (
            zz * (rng.beta(a, B) * 2 - 1) * a_e
        )  # ; # initialize Decision Variable with std so that performance is around 60%
        x_e = (
            Resp_vec[i - 1] * Block_vec[i] * x_e
        )  # ; %aftercorrect fit, so that it is really categ
        k = np.floor(t_0_u / dt)  # ; % initialize DDM iteration
        while (x_u < a_u) & (
            k * dt < t_0_e
        ):  # % for FBs and Express Responses (purely urgency-triggered responses)
            x_u = (
                x_u + vv_u * dt + dt ** 0.5 * rng.normal(size=1)[0]
            )  # ; % urgency integrator
            k = k + 1
        while (x_u < a_u) & (abs(x_e) < a_e):  # % for Non-Express Responses
            ...
            x_e = (
                x_e + v_e * dt + dt ** 0.5 * rng.normal(size=1)[0]
            )  # % evidence integrator
            k = k + 1  # ; % time step counter
        # end
        RT[i] = (
            k * dt
        )  # ; % the RT is already set, but the choice is not set yet for urgency-triggered trials
        bound[i] = (
            x_u < a_u
        )  # ; % whether the trial hit the evidence bound or the urgency bound
        if (not bound[i]) & (
            StimOnset < RT[i]
        ):  # % for urgency-triggered trials, integrate evidence for "(t_0_e-StimOnset)" seconds more
            T = RT[i]  # ; % we save the hitting time
            if T < t_0_e:  # % for express responses
                k = int(
                    t_0_e / dt
                )  # ; % we cannot integrate evidence until "t_0_e" # should I int or floor'?
            while k * dt < T + (
                t_0_e - StimOnset
            ):  # % keep integrating for "(t_0_e-StimOnset)" seconds more
                x_e = (
                    x_e + v_e * dt + dt ** 0.5 * rng.normal(size=1)[0]
                )  # ; % evidence integrator
                k = k + 1  # ; % time step counter
...
def D2M_simulation_parallel(args, kwargs={}):
    """this one should be direct transcript, later perhaps try to tune performance
    ---------------------------------
    %D2M simulations under parameters
    %"v_u","a_u","t_0_u","v_e","a_e","z_e","t_0_e". The numerical error allowed
    ...
    prep_rep = kwargs.get("prep_rep", 0.8)
    prep_alt = kwargs.get("prep_alt", 0.2)
    ...
    for i in range(1, n):
        ...
        while (x_u < a_u) & (
            ...
            x_u = (
                x_u + vv_u * dt + dt ** 0.5 * precomp_normal[k - offset_k]
            )  # rng.normal(size=1)[0] #; % urgency integrator
            k = k + 1
        precomp_normal = rng.normal(size=20000).reshape(2, -1)
        ii = 0
        while (x_u < a_u) & (abs(x_e) < a_e):  # % for Non-Express Responses
            x_u = (
                x_u + vv_u * dt + dt ** 0.5 * precomp_normal[0, ii]
            )  # rng.normal(size=1)[0] # % urgency integrator
            x_e = (
                x_e + v_e * dt + dt ** 0.5 * precomp_normal[1, ii]
            )  # rng.normal(size=1)[0] # % evidence integrator
            k = k + 1  # ; % time step counter
            ii += 1
        # end
        RT[i] = (
...
def parallel_wrapper(params, workers=8, seeds=np.arange(80), jobsperworker=10):
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        jobs = [executor.submit(D2M_simulation_parallel, params + [x]) for x in seeds]
        for job in tqdm.tqdm_notebook(
            as_completed(jobs), total=jobsperworker * workers
        ):
            out += [job.result()]

    RT = np.concatenate([out[x][0] for x in range(len(out))])
    v = np.concatenate([out[x][1] for x in range(len(out))])
    Accu = np.concatenate([out[x][2] for x in range(len(out))])
    bound = np.concatenate([out[x][3] for x in range(len(out))])
    s = np.concatenate([out[x][4] for x in range(len(out))])
