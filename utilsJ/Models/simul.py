...
def get_when_t(a, b, startfrom=700, tot_iter=1000, pval=0.001, nan_policy='omit'):
    ...
    for i in range(tot_iter):
        t2, p2 = ttest_ind(a[:,startfrom+i],b[:,startfrom+i], nan_policy=nan_policy)
        if p2<pval:
...
def when_did_split_dat(df, side,rtbin=0, rtbins=np.linspace(0,150,7), startfrom=700):
    ...
    if side==0:
        ...
        coh1 = 1
    dat = df.loc[(df.sound_len<rtbins[rtbin+1])&(df.sound_len>=rtbins[rtbin]) &(df.resp_len)] # &(df.R_response==side)
    mata = np.vstack(dat.loc[dat.coh2==coh1].swifter.apply(lambda x: plotting.interpolapply(x), axis=1).values.tolist())
    matb = np.vstack(dat.loc[(dat.coh2==0)& (dat.rewside==side)].swifter.apply(lambda x: plotting.interpolapply(x), axis=1).values.tolist())
...
def when_did_split_simul(df, side,rtbin=0, rtbins=np.linspace(0,150,7)):
    ...
    if side==0:
        ...
        coh1 = 1
    dat = df.loc[(df.sound_len<rtbins[rtbin+1])&(df.sound_len>=rtbins[rtbin])&(df.resp_len)] # &(df.R_response==side) this goes out
    mata = np.vstack(dat.loc[(dat.traj.apply(len)>0)&(dat.coh2==coh1), 'traj'].apply(shortpad).values.tolist())
    matb = np.vstack(dat.loc[(dat.traj.apply(len)>0)&(dat.coh2==0) & (dat.rewside==side), 'traj'].apply(shortpad).values.tolist())
...
def splitplot(df, out, ax):
    ...
    for i, (dat, name, marker) in enumerate([[tdf,'data', 'o'],[tdf2, 'simul', 'x']]):
        for j, side in enumerate(['L', 'R']):
            ax.scatter(dat.columns, dat.loc[j,:].values, marker=marker, color=colors[j], label=f'{name} {side}')
...
def plot0(df, out, ax):
    df.loc[(df.sound_len<250)].sound_len.hist(bins=np.linspace(0,250,101), ax=ax, label='data',  density=True, alpha=0.5, grid=False)
    out.sound_len.hist(bins=np.linspace(0,250,101), ax=ax, label='simul all', density=True, alpha=0.5,grid=False)
    ax.set_xlabel('RT (ms)')
    ax.set_title('RT distr')
    ax.legend(frameon=False, fancybox=False)

def pcomRT(df, out, ax):
    plotting.binned_curve(df, 'CoM_sugg', 'sound_len', bins=np.linspace(0,150,16),
                xpos=10, xoffset=5, ax=ax, errorbar_kw=dict(label='data', color='tab:blue'), legend=False, traces='sstr', traces_kw = dict(color='grey', alpha=0.3, ls='-'))
    plotting.binned_curve(out, 'CoM_sugg', 'sound_len', bins=np.linspace(0,150,16),
                xpos=10, xoffset=5, ax=ax, errorbar_kw=dict(label='simul', color='tab:orange'),legend=False, traces='sstr', traces_kw = dict(color='grey', alpha=0.3, ls=':'))
    ax.set_ylabel('p(CoM)')
    ax.set_xlabel('RT(ms)')
...
def plot2(df, out, ax):
    df.resp_len.hist(bins=np.linspace(0,1,81), ax=ax, label='data',  density=True, alpha=0.5, grid=False)
    out.resp_len.hist(bins=np.linspace(0,1,81), ax=ax, label='simul all', density=True, alpha=0.5,grid=False)
...
def plot3(df, out, ax):
    ...
    for i, dfobj in enumerate([
        ...
        plotting.binned_curve(
            ...
            xpos=10, traces='sstr', traces_kw = dict(color='grey', alpha=0.3, ls=traces_ls[i]),
            xoffset=5, errorbar_kw={'ls':'none', 'marker':'o','label':titles[i]}
        )
        ax.set_xlabel('RT (ms)')
        ax.set_ylabel('MT (s)')
...
def plot4(df,out,ax):
    ...
    counts_p ,_ = np.histogram(out.loc[out.reactive==0,'sound_len'], bins=np.linspace(0,250,26))
...
def plot67(df, out, ax,ax2, rtbins=np.linspace(0,150,7)):
    ...
    for i, (dat,store) in enumerate([[df, datres],[out, simulres]]):
        dat['priorbin']=pd.cut(dat.choice_x_allpriors,priorbins,labels=False, include_lowest=True)
        for j in range(rtbins.size-1):
            tmp = dat[(dat.sound_len>=rtbins[j])&(dat.sound_len<rtbins[j+1])].groupby('priorbin')['time_to_thr'].agg(m='mean',e=sem)
            store[f'rtbin{j}']=tmp['m']

            if j%2==0: # plot half of it
                kws={'ls': 'none', 'marker':markers[i], 'color':cmap(j/(rtbins.size-1)), 'capsize':2}
                if not i:
                    kws={'ls': 'none', 'marker':markers[i], 'label':f'rtbin={j}', 'color':cmap(j/(rtbins.size-1)), 'capsize':2}
                ax.errorbar(
                    tmp.index, tmp['m'], yerr=tmp['e'], **kws
                )
    ax.legend().remove()
    ...
    for i in range(rtbins.size-1):
        ax2.scatter([-2,-1,0,1,2], diffdf[f'rtbin{i}'].values, color=cmap(i/(rtbins.size-1)), label=f'rtbin {i}')
...
def plot910(df,out,ax,ax2,rtbins=np.linspace(0,150,7)):
    ...
    for i, (dat,store) in enumerate([[df, datres],[out, simulres]]):
        ...
        for j in range(rtbins.size-1):
            tmp = dat[(dat.sound_len>=rtbins[j])&(dat.sound_len<rtbins[j+1])].groupby('choice_x_coh')['time_to_thr'].agg(m='mean',e=sem)
            store[f'rtbin{j}']=tmp['m']
            if j%2==0:
                c=cmap(j/(rtbins.size-1))
                ax.errorbar(
                    tmp.index, tmp['m'], yerr=tmp['e'], **kwargs, c=c
                )
    ...
    for i in range(rtbins.size-1):
        ax2.scatter(diffdf.index, diffdf[f'rtbin{i}'].values, color=cmap(i/(rtbins.size-1)), label=f'rtbin {i}')

    ax2.legend(frameon=False, fancybox=False)
    
def plot1112(df, out, ax, ax2):
    subset = df.dropna(subset=['avtrapz', 'allpriors', 'CoM_sugg'])
    plotting.com_heatmap(subset.allpriors, subset.avtrapz,subset.CoM_sugg, flip=True, ax=ax, cmap='viridis', fmt='.0f')
    ax.set_title(f'real p(CoM)')
    subset = out.dropna(subset=['avtrapz', 'allpriors', 'CoM_sugg'])
    plotting.com_heatmap(subset.allpriors, subset.avtrapz,subset.CoM_sugg, flip=True, ax=ax2, cmap='magma', fmt='.0f')
...
def whole_simul(
    ...
    rtbins=np.linspace(0,150,7),
    params = {
        't_update':80, 'proact_deltaMT': 0.3, 'reactMT_interc': 110, 'reactMT_slope':0.15,
    ...
    df.loc[(df.R_response==1)&(df.trajectory_y.apply(len)>10),'time_to_thr'] = df.loc[(df.R_response==1)&(df.trajectory_y.apply(len)>10)].dropna(subset=['sound_len']).swifter.apply(lambda x: np.argmax(plotting.interpolapply(x)[700:]>30), axis=1) # axis arg not req. in series
    df.loc[(df.R_response==0)&(df.trajectory_y.apply(len)>10),'time_to_thr'] = df.loc[(df.R_response==0)&(df.trajectory_y.apply(len)>10)].dropna(subset=['sound_len']).swifter.apply(lambda x: np.argmax(plotting.interpolapply(x)[700:]<-30), axis=1) # axis arg not req. in series
    df['rtbin']=pd.cut(df.sound_len,rtbins,labels=False, include_lowest=True)
    df['choice_x_coh'] = (df.R_response*2-1) * df.coh2
    df['allpriors'] = np.nansum(df[['dW_trans', 'dW_lat']].values, axis=1) # , 'dW_fixedbias'
    df['choice_x_allpriors'] = (df.R_response*2-1) * df.allpriors

    psiam_params = loadmat(f'/home/jordi/DATA/Documents/changes_of_mind/data/paper/fits_psiam/{subject} D2Mconstrainedfit_fitonly.mat')['freepar_hat'][0]
    ...
    if sample_silent_only:
        ...
        print(f'just sampling from silent trials in subject {subject}\nwhich is around {len(df.loc[df.subjid==subject])}')
    with ThreadPoolExecutor(max_workers=7) as executor:
        jobs = [
            executor.submit(_callsimul, 
        ...
        1.3,0.3,1e-4,x, batches, batch_size, params['glm2Ze_scaling'],
        silent_trials, sample_silent_only, params['x_e0_noise'],
        params['confirm_thr'], params['proportional_confirm']]) 
    ...
    try: ### PROACTIVE RESPONSES
        ...
        if mtnoise: # load error
            with open(f'/home/jordi/DATA/Documents/changes_of_mind/data/paper/trajectory_fit/MTmse.pkl', 'rb') as handle:
        ...
        if params['naive_jerk']:
            naive_jerk = np.array([0,0,0,75,0,0]).reshape(-1,1) # broadcast final position to port # since it is aligned to final choice, that-s it. 
        ...
        fkmat = sdf[['zidx', 'dW_trans_i', 'dW_lat_i']].fillna(0).values
        fkmat = np.insert(fkmat, 0, 1,axis=1)
        times = sdf.loc[sdf.index, 'origidx'].values*params['reactMT_slope'] + params['reactMT_interc']
        ...
        if params['naive_jerk']:
            ...
            out.loc[sdf.index,f'mu_boundary'] = tr.return_mu(Fk=fkmat) # this one is not used later, right?
    ...
    out.loc[out.sound_len<remaining_sensory, 'remaining_sensory'] = out.loc[out.sound_len<remaining_sensory, 'sound_len'] 
    out.loc[(out.e_time*1000<out.sound_len+out.remaining_sensory)&(out.reactive==0), 'e_after_u'] = 1 # control for those which have listened less than the delay # done
    ...
    if not vanishing_bounds:
        ...
        out.loc[out.e_after_u==1, 't_update'] = (out.loc[out.e_after_u==1, 'e_time'] - remaining_sensory) + params['t_update'] # e_time is already respective sound onset
    ...
    if not silent_trials:
        if params['confirm_thr']>0:
            out.loc[(out.R_response==out.prechoice)&(out.reactive==0),  'resp_len'] = (1-params['proact_deltaMT'] *out.loc[(out.R_response==out.prechoice)&(out.reactive==0), 'delta_ev'].abs()/a_e ) * (out.loc[(out.R_response==out.prechoice)&(out.reactive==0), 'resp_len'] - out.loc[(out.R_response==out.prechoice)&(out.reactive==0),  't_update']) +\
                                                                        out.loc[(out.R_response==out.prechoice)&(out.reactive==0),  't_update']   # prechoice==final choice aka confirm
        else:
            out.loc[(out.R_response==out.prechoice)&(out.reactive==0),  'resp_len'] = (1-params['proact_deltaMT'] *out.loc[(out.R_response==out.prechoice)&(out.reactive==0), 'x_e'].abs()/a_e ) * (out.loc[(out.R_response==out.prechoice)&(out.reactive==0), 'resp_len'] - out.loc[(out.R_response==out.prechoice)&(out.reactive==0),  't_update']) +\
                                                                        out.loc[(out.R_response==out.prechoice)&(out.reactive==0),  't_update']   # prechoice==final choice aka confirm
        ...
        out.loc[(out.R_response!=out.prechoice)&(out.reactive==0),  'resp_len'] =  out.loc[(out.R_response!=out.prechoice)&(out.reactive==0),  't_update'] + params['com_gamma'] + params['reactMT_slope'] * out.loc[(out.R_response!=out.prechoice)&(out.reactive==0),'origidx'] # tri_ind is new
    ...
    if both_traj:
        ...
        out[['pretraj', 'traj']] = out.swifter.apply(
            lambda x: traj.simul_traj_single(x, return_both=True, silent_trials=silent_trials,
            trajMT_jerk_extension=trajMT_jerk_extension), axis=1, result_type='expand'
            )
    else:
        out['traj'] = out.swifter.apply(lambda x: traj.simul_traj_single(x, silent_trials=silent_trials,
        trajMT_jerk_extension=trajMT_jerk_extension), axis=1)

    tmp = out.apply(lambda x: plotting.gradient_np_simul(x), axis=1, result_type='expand') # add swifter
    ...
    for i, thr in enumerate([30,30]):
        out.loc[(out.R_response==i)&(out.traj.apply(len)>0), 'time_to_thr'] = out.loc[(out.R_response==i)&(out.traj.apply(len)>0), 'traj'].swifter.apply(lambda x: np.argmax(np.abs(x)>thr))
