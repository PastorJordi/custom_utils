...
savpath = '/home/jordi/Documents/changes_of_mind/firstanal_com_new_setup/all_data_so_far/'
SubjectsToUpdate = ['LE82', 'LE84', 'LE85', 'LE86', 'LE87']
UpdateWholeDataset = True
ReuseData = False # whether to reuse individual subjects pkl files or reanalyze everything
mask = 'sound_len <= 400 and soundrfail == False and resp_len <=1 and R_response>= 0' \
    +' and hithistory >= 0 and special_trial == 0' # for glm analysis, all trials remain in df
pipe_kwargs = dict(
...
if ReuseData:
    ...
    for subj in tqdm.tqdm(SubjectsToUpdate):
        try:
            ...
            df = df.loc[df.sessid.str.contains('leftright|nofixnopain|onlylight')==False]
            for col in newcols:
                df.loc[:,col]=np.nan
            ...
            if len(df.loc[(df.subjid==subj)&df.sessid.str.contains('feedback')]):
                for bo in [False, True]:
                    ...
                    df.loc[cur_mask] = glm2afc.piped_moduleweight(
                            df.loc[cur_mask], filtermask=mask, 
                            lateralized=lateral_flag, noenv=noenv_flag
                        )
            elif subj not in [f'LE{x}' for x in range(82,88)]:
                df.loc[df.subjid==subj] = glm2afc.piped_moduleweight(
                        df.loc[(df.subjid==subj)], filtermask=mask, 
                        noenv=False, lateralized=True
                    )
            else:
                df.loc[df.subjid==subj] = glm2afc.piped_moduleweight(
                        df.loc[(df.subjid==subj)], filtermask=mask, 
                        noenv=True, lateralized=False
                    )        
...
for subj in pbar:
    ...
    try:
        df = pd.concat(
            [df, pd.read_pickle(savpath+subj+'.pkl')], 
