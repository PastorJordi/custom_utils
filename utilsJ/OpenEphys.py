...
def loadFolderToArray(
    ...
    if chprefix:  # ideally chprefix should be a list if containing several prefixes
        try:
            if session == "0":
                filelist = [
                    source + "_" + chprefix + x + ".continuous"
...
def pack_3(
    ...
    channel_1_data = loadContinuous(
        os.path.join(folderpath, files[0]), dtype, verbose=False
    )["data"]
    ...
    for i, f in enumerate(pbar):
        ...
        data_array[:, i + 1] = loadContinuous(
            os.path.join(folderpath, f), dtype, verbose=False
        )["data"]
        numFiles += 1
