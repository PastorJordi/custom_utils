...
def fig_bernstein(df, data_folder=DATA_FOLDER, task_img=TASK_IMG, model_img=MODEL_IMG,
    ...
    # MT VS PRIOR
    df_mt = df.copy()
    fig_1.plot_mt_vs_evidence(df=df_mt.loc[df_mt.special_trial == 2], ax=ax_mt_coh,
                              condition='choice_x_prior', prior_limit=1,
                              rt_lim=200)
    del df_mt
    # MT VS COH
    df_mt = df.copy()
    fig_1.plot_mt_vs_evidence(df=df_mt, ax=ax_mt_zt, prior_limit=0.1,  # 10% quantile
                              condition='choice_x_coh', rt_lim=50)
    del df_mt
    # REGRESSION WEIGHTS
    df_wghts = df.copy()
    fig_1.mt_weights(df=df_wghts, ax=ax[3], plot=True, means_errs=False)
    del df_wghts
