    ...
    if not average:
        for subject in df.subjid.unique():
            ...
            if not collapse_sides:
                ...
                split_time_0 = simul.when_did_split_dat(
                    df[df.subjid==subject], 0, collapse_sides=True, ax=ax,
                    plot_kwargs=dict(color='tab:green', label='rtbin=0')
                )
                split_time_4 = simul.when_did_split_dat(
                    df[df.subjid==subject], 1, ax=ax,
                    collapse_sides=True, rtbin=4,
                    plot_kwargs=dict(color='tab:blue', label='rtbin=4')
                )
                ax.set_xlim(-10,150)
...
def d(df, collapse_sides=False, rtbins=np.linspace(0,150,16), 
    ...
    out_data = np.array(out_data).reshape(df.subjid.unique().size, rtbins.size-1, -1)
    ...
    if not connect_points:
        ax.scatter( # add some offset/shift on x axis based on binsize
            ...
            ) + np.random.normal(loc=0, scale=0.2, size=out_data.size)), # adding some jitter
            out_data.flatten(),
            **scatter_kws,
        )
    else:
    ...
    error_kws = dict(ecolor='k', capsize=2, mfc=(1,1,1,0), mec='k', color='k', marker='o', 
                label='mean & SEM')
    ax.errorbar(
        binsize/2 + binsize * np.arange(rtbins.size-1),
        np.nanmean(out_data.reshape(rtbins.size-1, -1), axis=1), # we do the mean across rtbin axis
        # other axes we dont care
        yerr=sem(out_data.reshape(rtbins.size-1, -1), axis=1, nan_policy='omit'),
        **error_kws
    )
